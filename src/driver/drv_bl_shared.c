...
// Expanded to 48 slots (12-hour circular buffer)
static int consumption_matrix [48] = {0}; 
static int export_matrix[48] = {0};
static int net_matrix[48] = {0};
...
// FIX: Explicitly include the HTTP server header to fix compilation error
...
void BL09XX_AppendInformationToHTTPIndexPage(http_request_t *request)
{
    ...
    // Safe Hide: Only hides OpenBeken's default relay tables, protects our custom tables
    poststr(request, "#state > table:not(.my-tbl) { display: none; }"); 
    poststr(request, ".my-tbl { display: table !important; width:100%; text-align:center; font-size:16px; margin:10px 0; table-layout:fixed; }");
    poststr(request, ".my-tbl th { color:#aaa; font-weight:normal; padding-bottom:5px; }");
    poststr(request, ".my-tbl td { padding-bottom:10px; }");
    
    // Graph CSS exactly matching the 480x201 requirement
    poststr(request, ".g-wrap { display:flex; height:201px; max-width:530px; margin:20px auto; font-family:sans-serif; }");
    poststr(request, ".g-y { width:45px; display:flex; flex-direction:column; justify-content:space-between; font-size:10px; color:#aaa; text-align:right; padding-right:8px; }");
    poststr(request, ".g-box { width:480px; display:flex; position:relative; background:#2a2a2a; border-radius:4px; }");
    poststr(request, ".g-line { position:absolute; top:60px; width:100%; height:1px; background:#666; z-index:5; }"); // Line at pixel 61
    poststr(request, ".g-b { width:20px; height:100%; display:flex; flex-direction:column; }");
    
    // Top area (60px space above line). Bottom area (140px space below line).
    poststr(request, ".g-top { height:60px; position:relative; }");
    poststr(request, ".g-bot { height:140px; position:relative; }");
    
    // Green Export (Grows UP from bottom of the top 60px block)
    poststr(request, ".g-up { position:absolute; bottom:0; width:100%; background:#2ecc71; display:flex; align-items:center; justify-content:center; overflow:hidden; }");
    // Red Import (Grows DOWN from top of the bottom 140px block)
    poststr(request, ".g-dn { position:absolute; top:0; width:100%; background:#e74c3c; display:flex; align-items:center; justify-content:center; overflow:hidden; }");
    ...
    // ====================================================================
    // 1. HORIZONTAL DASHBOARD (4 Columns)
    // ====================================================================
    poststr(request, "<table class='my-tbl'><tr>");
    poststr(request, "<th>Voltage</th><th>Power</th><th>15-Min Est.</th><th>Charger C</th></tr><tr>");
    
    hprintf255(request, "<td><b>%.0f V</b></td><td><b>%.0f W</b></td><td><b>%i Wh</b></td><td><b style='color:#0099FF;'>%i%%</b></td></tr>", 
               sensors[OBK_VOLTAGE].lastReading, sensors[OBK_POWER].lastReading, estimated_energy_period, dump_load_relay[5]);
    ...
    if (CFG_HasFlag(OBK_FLAG_POWER_ALLOW_NEGATIVE) && NTP_IsTimeSynced())
    {
        ...
        // ====================================================================
        // 2. LIVE NETMETERING BUCKET (Current Period)
        // ====================================================================
        int buffer_index = current_interval_of_day % 48;
        int hour = current_interval_of_day / 4;
        int minute = (current_interval_of_day % 4) * 15;
        ...
        poststr(request, "<table class='my-tbl' style='font-size:14px;'><tr><th style='text-align:left;'>Time</th><th>Import</th><th>Export</th><th>Net</th></tr>");
        hprintf255(request, "<tr><td style='text-align:left;'><b>%02i:%02i</b></td><td><b>%dW</b></td><td><b>%dW</b></td><td><b>%dW</b></td></tr></table><hr>", 
                   hour, minute, disp_cons, disp_exp, disp_net);

        // ====================================================================
        // 3. THE 480x201 BAR GRAPH (LAST 6 HOURS / 24 BARS)
        // ====================================================================
        poststr(request, "<h2 style='font-size:18px; color:#eaeaea; text-align:left; max-width:530px; margin:auto;'>Energy Stats (Last 6 Hours)</h2>");
        poststr(request, "<div class='g-wrap'><div class='g-y'><span>+700W</span><span>0W</span><span>-300W</span></div>");
        poststr(request, "<div class='g-box'><div class='g-line'></div>");

        // Draw 24 horizontal slots, 20px each
        for (int i = 23; i >= 0; i--) {
            int interval_of_day = current_interval_of_day - i;
            if (interval_of_day < 0) { interval_of_day += 96; } 
            int c_index = interval_of_day % 48;
            
            int v = net_matrix[c_index];
            if (i == 0) { v += (int)(real_consumption - real_export); } // Add live to newest bar
            
            poststr(request, "<div class='g-b'>");
            
            if (v < 0) {
                // Export (Negative): Green bar growing UP in the 60px top slot. 100px = 300W.
                int h_up = (abs(v) * 100) / 300;
                if (h_up > 60) h_up = 60;
                if (h_up < 1) h_up = 1; 
                hprintf255(request, "<div class='g-top'><div class='g-up' style='height:%dpx;'><span class='b-txt'>%d</span></div></div><div class='g-bot'></div>", h_up, abs(v));
            } else if (v > 0) {
                // Import (Positive): Red bar growing DOWN in the 140px bottom slot. 100px = 700W.
                int h_dn = (v * 100) / 700;
                if (h_dn > 140) h_dn = 140;
                if (h_dn < 1) h_dn = 1;
                hprintf255(request, "<div class='g-top'></div><div class='g-bot'><div class='g-dn' style='height:%dpx;'><span class='b-txt'>%d</span></div></div>", h_dn, v);
            } else {
                // Zero
        ...
        poststr(request, "</div></div><hr>");

        // ====================================================================
        // 4. 12-HOUR HISTORY TABLE
        // ====================================================================
        poststr(request, "<table class='my-tbl' style='font-size:14px; text-align:center;'>");
        poststr(request, "<tr><th style='text-align:left;'>Time</th><th>Import</th><th>Export</th><th>Net</th></tr>");

        // Loop the remaining 47 intervals (last 12 hours minus the active one)
        for (int i = 1; i < 48; i++) { 
            int interval_of_day = current_interval_of_day - i;
            if (interval_of_day < 0) { interval_of_day += 96; } 
            
            int c_index = interval_of_day % 48;
            int h = interval_of_day / 4;
            int m = (interval_of_day % 4) * 15;
            
            int d_net = net_matrix[c_index];
            hprintf255(request, "<tr><td style='text-align:left;'>%02i:%02i</td><td>-</td><td>-</td><td>%dW</td></tr>", h, m, d_net);
        }
        poststr(request, "</table>");
    }

    // ====================================================================
    // 5. DETAILED SENSOR DATA
    // ====================================================================
    poststr(request, "<hr><h3 style='font-size:16px; color:#eaeaea; margin-top:20px;'>Detailed Sensor Data</h3>");
    poststr(request, "<table class='my-tbl' style='font-size:14px; text-align:left;'>");
    ...
    for (int i = (OBK__FIRST); i <= (OBK_CONSUMPTION__DAILY_LAST); i++) {
        ...
        if (i <= OBK__NUM_MEASUREMENTS || NTP_IsTimeSynced()) {
            ...
            // Skip Voltage/Power/Current as they are handled cleanly at the top
            if (i == OBK_VOLTAGE || i == OBK_POWER || i == OBK_CURRENT) continue;
    ...
    poststr(request, "</table>");
    hprintf255(request, "<font size=1>Saving Interval: %.2fW</font><br><br>", changeSavedThresholdEnergy);
}
...
void BL_ProcessUpdate(float voltage, float current, float power, float frequency, float energyWh) {
    ...
    {                                          
        ...
        // Maps the interval to our expanded 48-slot circular buffer
        int current_matrix_index = interval_of_day_tracker % 48; 
        ...
        if (current_matrix_index != last_matrix_index) {
            ...
            // Wipe the incoming block to destroy data from 12 hours ago
