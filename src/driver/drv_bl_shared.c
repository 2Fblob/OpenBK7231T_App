...
void BL09XX_AppendInformationToHTTPIndexPage(http_request_t *request)
{
    ...
    // New Controls Layout
    poststr(request, ".ctrl-col { flex: 0 0 90px; background: #222; padding: 10px; border-radius: 8px; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 10px; }");
    poststr(request, ".btn-tgl { width: 100%; padding: 12px 0; border: none; border-radius: 4px; font-weight: bold; cursor: pointer; color: white; background: #0099FF; transition: background 0.3s; }");
    poststr(request, ".slider { -webkit-appearance: none; width: 100%; height: 6px; border-radius: 3px; background: #444; outline: none; }");
    ...
    if (CFG_HasFlag(OBK_FLAG_POWER_ALLOW_NEGATIVE) && NTP_IsTimeSynced())
    {
        ...
        // ====================================================================
        // 3. CONTROLS (Middle Column - Max 100px width)
        // ====================================================================
        poststr(request, "<div class='ctrl-col'>");
        poststr(request, "<div style='font-size:10px; color:#888; text-transform:uppercase;'>Mode</div>");
        poststr(request, "<button id='m-btn' class='btn-tgl' onclick='tm()'>AUTO</button>");
        
        poststr(request, "<div style='font-size:10px; color:#888; text-transform:uppercase; margin-top:15px;'>Charger C</div>");
        ...
        // Inline JS handler for toggle button
        ...
        // ====================================================================
        // 4. CANVAS BAR GRAPH (Right Column) - CLIENT SIDE RENDERED
        // ====================================================================
        ...
        // Output a blank Canvas element matching the old 352x260 viewbox. CSS will automatically shrink it.
        ...
        // --- Table Section (Combined Import/Export) ---
        poststr(request, "<div class='hist-tbl-wrapper' style='flex:1; min-width:280px; margin-top:0;'>");
        ...
        // --- Big Clock Section (Maximized Font, No Title) ---
        poststr(request, "<div style='flex:0 0 240px; display:flex; justify-content:center; align-items:center; background:#222; border-radius:8px; padding:10px; overflow:hidden;'>");
        hprintf255(request, "<div style='font-size:68px; font-weight:bold; color:#0099FF; font-family:monospace; line-height:1; letter-spacing:-2px;'>%02d:%02d</div>", NTP_GetHour(), NTP_GetMinute());
        poststr(request, "</div>");
