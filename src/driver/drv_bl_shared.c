...
void BL09XX_AppendInformationToHTTPIndexPage(http_request_t *request)
{
	...
	if (CFG_HasFlag(OBK_FLAG_POWER_ALLOW_NEGATIVE))
	{
		...
		// Render the high precision 24-slot SVG Canvas
		...
		// Static precise middle horizontal zero-line scaled to 0-300-700
		...
		if (NTP_IsTimeSynced())
		{
			...
			for (int i = 23; i >= 0; i--)
			{
				...
				if (v < 0) {
					// Export Side (Green Bars, grows UP). Scale: 90px max / 300Wh
					int h = (abs(v) * 90) / 300;
					if (h > 90) h = 90;
					if (h < 1) h = 1;
					hprintf255(request, "<rect x='%d' y='%d' width='18' height='%d' fill='#2ecc71' />", x_pos + 1, 90 - h, h);
					hprintf255(request, "<text x='%d' y='85' fill='#fff' font-size='9' transform='rotate(-90 %d 85)'>%d</text>", x_pos + 13, x_pos + 13, abs(v));
				} 
				else if (v > 0) {
					// Import Side (Red Bars, grows DOWN). Scale: 210px max / 700Wh
					int h = (v * 210) / 700;
					if (h > 210) h = 210;
					if (h < 1) h = 1;
					hprintf255(request, "<rect x='%d' y='90' width='18' height='%d' fill='#e74c3c' />", x_pos + 1, h);
					hprintf255(request, "<text x='%d' y='85' fill='#fff' font-size='9' transform='rotate(-90 %d 85)'>%d</text>", x_pos + 13, x_pos + 13, v);
				}

				total_consumption += consumption_matrix[c_index];
				total_export += export_matrix[c_index];

				if (net_matrix[c_index] < 0) {
					total_net_export -= net_matrix[c_index];
				} else {
					total_net_consumption += net_matrix[c_index];
				}
			}
		...
		hprintf255(request, "<font size=2 color=#0099FF>- Charger C PWM: <b>%i%%</b> <br></font>", charger_c_pwm_debug);
	}
	...
	{	
		...
		hprintf255(request,"<h5>NetMetering (Last %d min out of %d): %.3f Wh</h5><hr>", energyCounterMinutesIndex, energyCounterSampleCount, net_energy); 
	}
}
...
void BL_ProcessUpdate(float voltage, float current, float power,
	...
	{			
		...
		// Tracks rolling 15-minute interval blocks across the 24 array structures
		int minutes_since_midnight_tracker = (check_hour * 60) + check_time;
		int current_matrix_index = (minutes_since_midnight_tracker / 15) % 24;
		...
		export_matrix[current_matrix_index] = old_export_energy + (int)real_export;
		consumption_matrix[current_matrix_index] = old_real_consumption + (int)real_consumption;		

		if (hour_reset == 1)
		{
			lastsync = 0;

			if (current_matrix_index > 0) { net_matrix[current_matrix_index - 1] += net_energy; }
		...
		// Modified threshold for solar available tracking from 100 to 25
		if (net_energy < -25) {
			solar_available = 1; 
		...
		if (current_minute != last_minute) 
		{
			...
			if (current_minute > 0)
			{
				...
				else 
				{
					...
					{
						int rem = 60 - current_minute; 
						net_energy_equivalent = (int)((float)net_energy * (60.0f / (float)rem));
					}
				}
			}
			if (check_time > 10)
			...
			if (net_energy > 0) {
				...
				int projW = -net_energy_equivalent;       
				if (projW < 0) projW = 0;                  
				int p = projW / 10;                        
				if (p < 10)  p = 10;                      
				if (p > 100) p = 100;                     
			...
			// ======================================================================
			// SAFE TEST: HTTP SEND DISABLED, TASMOTA UDP FOR CHARGER C ADDED
			// ======================================================================
			for (int output_index = 0; output_index < dump_load_relay_number; output_index++) 
			{
				if (dump_load_relay[output_index] != last_dump_load_relay[output_index]) 
				{
					...
					if (dump_load_relay_ip[output_index] == charger_c_ip) 
					{
						...
						// --- NEW TASMOTA UDP BROADCAST ---
						// This executes for Charger C when it changes.
						snprintf(output_command, sizeof(output_command), "DGR_SendDimmer solar_dump %d", dump_load_relay[output_index]);
						CMD_ExecuteCommand(output_command, 0);
						addLogAdv(LOG_INFO, LOG_FEATURE_MAIN, "SENDING UDP: %s", output_command);
					}
					else
					{
						ip_middle = "/cm?cmnd=Power%20"; 
					}
				
					// --- ORIGINAL SENDGET ROUTINE COMMENTED OUT ---
					// By keeping this commented out, we send nothing for the other relays,
					// but the state tracking above remains completely unchanged.
					/*
