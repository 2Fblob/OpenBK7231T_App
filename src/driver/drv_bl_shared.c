    ...
    if (CFG_HasFlag(OBK_FLAG_POWER_ALLOW_NEGATIVE) && NTP_IsTimeSynced())
    {
        ...
        for (int i = 31; i >= 0; i--) {
            ...
            if (display_net >= 0) { 
                ...
                // Vertical Text ALWAYS drawn. Rotated -90deg points UP above the bar.
                hprintf255(request, "<text transform=\"translate(%d, %d) rotate(-90)\" fill=\"#fff\" font-size=\"10\" font-family=\"sans-serif\" dominant-baseline=\"middle\" text-anchor=\"start\">%dW</text>", x + 5, 230 - h - 4, net);
            } else {
                ...
                // Vertical Text ALWAYS drawn. Rotated 90deg points DOWN below the bar.
                hprintf255(request, "<text transform=\"translate(%d, %d) rotate(90)\" fill=\"#fff\" font-size=\"10\" font-family=\"sans-serif\" dominant-baseline=\"middle\" text-anchor=\"start\">%dW</text>", x + 5, 230 + h + 4, net);
            }
        }
