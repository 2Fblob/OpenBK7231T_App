...
{
    if(args && *args) { target_export = atoi(args); }
    return CMD_RES_OK;
}
...
commandResult_t BL09XX_SetupEnergyStatistic(const void *context, const char *cmd, const char *args, int cmdFlags)
{
    ...
    if (Tokenizer_GetArgsCount() >= 4) json_enable = Tokenizer_GetArgInteger(3);
    else json_enable = 0;
    ...
    if (enable != 0)
    {
        ...
        {
            if (energyCounterMinutes != NULL) os_free(energyCounterMinutes);
        ...
        if (energyCounterSampleInterval != sample_time)
        {
            ...
            if (energyCounterMinutes != NULL) memset(energyCounterMinutes, 0, energyCounterSampleCount*sizeof(float));
        }
        if (energyCounterMinutes == NULL)
        {
        ...
        if (energyCounterMinutes != NULL) { os_free(energyCounterMinutes); energyCounterMinutes = NULL; }
        energyCounterSampleCount = sample_count;
        energyCounterSampleInterval = sample_time;
    }
...
bool Channel_AreAllRelaysOpen() {
    ...
    for (i = 0; i < PLATFORM_GPIO_MAX; i++) {
        ...
        if (role == IOR_Relay) { if (CHANNEL_Get(ch)) { return false; } }
        if (role == IOR_Relay_n) { if (CHANNEL_Get(ch)==false) { return false; } }
        if (role == IOR_BridgeForward) { if (CHANNEL_Get(ch)) { return false; } }
    }
    return true;
}
...
void BL_ProcessUpdate(float voltage, float current, float power, float frequency, float energyWh) {
    int i; int xPassedTicks; float energy_counter_data = 0; float period_net = 0; int process_net_stats = 0;
    cJSON* root; cJSON* stats; char *msg; portTickType interval; time_t ntpTime; struct tm *ltm; char datetime[64]; float diff;
    ...
    {                                         
        ...
        if ((current_sys_tick - last_30s_tick) >= (30000 / portTICK_PERIOD_MS) || last_30s_tick == 0) {
            ...
            if (current_dmp >= 18 && current_dmp <= 100) { current_charger_c_accum += current_dmp; } 
            else if (current_dmp == 5) { current_inverter_accum += 100; }
            sample_count_30s++;
        }

        {
            ...
            if (last_matrix_index == -1) { last_matrix_index = current_matrix_index; }
            ...
            if (current_matrix_index != last_matrix_index) {
                ...
                real_export = 0; real_consumption = 0; net_energy = 0;
                energyCounterMinutesIndex = 0; lastsync = 0; 
                ...
                current_charger_c_accum = 0; current_inverter_accum = 0; sample_count_30s = 0;
                last_matrix_index = current_matrix_index; savetoflash = 1;
            }
        }

        if (!(check_time == old_time)) { min_reset = 1; old_time = check_time; lastsync++; }
        ...
        {
            int min_in_block; int check_time_estimate_mins; char fallback_cmd[64];
            ...
            if (min_in_block > 0) { net_energy_equivalent = (int)((float)net_energy * (15.0f / min_in_block)); } 
            else { net_energy_equivalent = (int)net_energy; }

            if (net_energy < -((float)target_export + 10.0f)) { solar_available = 1; } 
            else if (net_energy > 10.0f) { solar_available = 0; }
            ...
            if (charger_c_auto == 1) {
                ...
                if (solar_available == 0) {
                    if (net_energy < -10.0f) { persistent_state = 0; } 
                    else if (net_energy > 5.0f) { persistent_state = 5; }
                    solar_excess = 0; 
                } 
                else {
                    if (net_energy > -((float)target_export)) { solar_excess = 0; } 
                    else {
    ...
    if (CFG_HasFlag(OBK_FLAG_POWER_FORCE_ZERO_IF_RELAYS_OPEN)) {
        if (Channel_AreAllRelaysOpen()) { power = 0; current = 0; }
    }
    ...
    if (isnan(energyWh)) {
        ...
        if (xPassedTicks <= 0) xPassedTicks = 1;
        energy = xPassedTicks * power / (3600000.0f / portTICK_PERIOD_MS);
    } 
    else {
        if ((int)power>0) { real_consumption += energyWh; }
        else { if (CFG_HasFlag(OBK_FLAG_POWER_ALLOW_NEGATIVE)) { real_export += energyWh; } }
    }
    ...
    if (NTP_IsTimeSynced()) {
        ...
        ltm = gmtime(&ntpTime);
        if (ConsumptionResetTime == 0) ConsumptionResetTime = (time_t)ntpTime;

        if (actual_mday == -1) { actual_mday = ltm->tm_mday; }
    ...
    if (energyCounterStatsEnable == true) {
        ...
        if ((xTaskGetTickCount() - energyCounterMinutesStamp) >= interval) {
            ...
            if ((energyCounterStatsJSONEnable == true) && (MQTT_IsReady() == true)) {
                ...
                if(NTP_IsTimeSynced() == true) {
                    ...
                    if (NTP_GetTimesZoneOfsSeconds()>0) {
                        snprintf(datetime,sizeof(datetime), "%04i-%02i-%02iT%02i:%02i+%02i:%02i", ltm->tm_year+1900, ltm->tm_mon+1, ltm->tm_mday, ltm->tm_hour, ltm->tm_min, NTP_GetTimesZoneOfsSeconds()/3600, (NTP_GetTimesZoneOfsSeconds()/60) % 60);
                    } else {
                        snprintf(datetime, sizeof(datetime), "%04i-%02i-%02iT%02i:%02i-%02i:%02i", ltm->tm_year+1900, ltm->tm_mon+1, ltm->tm_mday, ltm->tm_hour, ltm->tm_min, abs(NTP_GetTimesZoneOfsSeconds()/3600), (abs(NTP_GetTimesZoneOfsSeconds())/60) % 60);
                    }
                ...
                if (energyCounterMinutes != NULL) {
                    ...
                    for(int k = 0; k < energyCounterSampleCount; k++) { cJSON_AddItemToArray(stats, cJSON_CreateNumber(energyCounterMinutes[k])); }
                    cJSON_AddItemToObject(root, "consumption_samples", stats);
                }

                if(NTP_IsTimeSynced() == true) {
                    stats = cJSON_CreateArray();
                    for(int m = OBK_CONSUMPTION__DAILY_FIRST; m <= OBK_CONSUMPTION__DAILY_LAST; m++) { cJSON_AddItemToArray(stats, cJSON_CreateNumber(DRV_GetReading(m))); }
            ...
            if (energyCounterMinutes != NULL) {
                for (int n=energyCounterSampleCount-1; n>0; n--) { energyCounterMinutes[n] = energyCounterMinutes[n-1]; }
                energyCounterMinutes[0] = 0.0;
            }
            energyCounterMinutesStamp = xTaskGetTickCount();
            energyCounterMinutesIndex++;
        }
        if (energyCounterMinutes != NULL) energyCounterMinutes[0] += energy_counter_data;
    }

    for(i = OBK__FIRST; i <= OBK__LAST; i++) {
        diff = sensors[i].lastSentValue - sensors[i].lastReading;
        if ( ((fabsf(diff) > sensors[i].changeSendThreshold) && (sensors[i].noChangeFrame >= changeDoNotSendMinFrames)) || (sensors[i].noChangeFrame >= changeSendAlwaysFrames) ) {
            enum EventCode eventChangeCode; sensors[i].noChangeFrame = 0;
            ...
            case CMD_EVENT_NONE: break;
            case CMD_EVENT_CHANGE_CURRENT: 
            {
                int prev_mA = sensors[i].lastSentValue * 1000; int now_mA = sensors[i].lastReading * 1000;
                EventHandlers_ProcessVariableChange_Integer(eventChangeCode, prev_mA,now_mA); break;
            }
            default:
                EventHandlers_ProcessVariableChange_Integer(eventChangeCode, sensors[i].lastSentValue, sensors[i].lastReading); break;
            }
            ...
            sensors[i].noChangeFrame++; stat_updatesSkipped++;
        }
    }       

    if (((((sensors[OBK_CONSUMPTION_TOTAL].lastReading - lastSavedEnergyCounterValue) >= changeSavedThresholdEnergy) || ((xTaskGetTickCount() - lastConsumptionSaveStamp) >= (6 * 3600 * 1000 / portTICK_PERIOD_MS)) || ((sensors[OBK_GENERATION_TOTAL].lastReading - lastSavedGenerationCounterValue) >= changeSavedThresholdEnergy)) && (!(CFG_HasFlag(OBK_FLAG_POWER_ALLOW_NEGATIVE))))||(savetoflash == 1))
    {
    savetoflash = 0;
#if WINDOWS
#elif PLATFORM_BL602
...
{
    int i; ENERGY_METERING_DATA data;

    for(i = OBK__FIRST; i <= OBK__LAST; i++) { sensors[i].noChangeFrame = 0; sensors[i].lastReading = 0; }
    energyCounterStamp = xTaskGetTickCount(); 

    if (energyCounterStatsEnable == true) {
        if (energyCounterMinutes == NULL) { energyCounterMinutes = (float*)os_malloc(energyCounterSampleCount*sizeof(float)); }
        if (energyCounterMinutes != NULL) { for(i = 0; i < energyCounterSampleCount; i++) { energyCounterMinutes[i] = 0.0; } }
        energyCounterMinutesStamp = xTaskGetTickCount(); energyCounterMinutesIndex = 0;
    }
    ...
    sensors[OBK_CONSUMPTION_TOTAL].lastReading = data.TotalConsumption; sensors[OBK_GENERATION_TOTAL].lastReading = data.TotalGeneration;
    sensors[OBK_CONSUMPTION_TODAY].lastReading = data.TodayConsumpion; sensors[OBK_CONSUMPTION_YESTERDAY].lastReading = data.YesterdayConsumption;
    actual_mday = data.actual_mday;   
    lastSavedEnergyCounterValue = data.TotalConsumption; lastSavedGenerationCounterValue = data.TotalGeneration;
    sensors[OBK_CONSUMPTION_2_DAYS_AGO].lastReading = data.ConsumptionHistory[0]; sensors[OBK_CONSUMPTION_3_DAYS_AGO].lastReading = data.ConsumptionHistory[1];
    ConsumptionResetTime = data.ConsumptionResetTime; ConsumptionSaveCounter = data.save_counter; lastConsumptionSaveStamp = xTaskGetTickCount();

    CMD_RegisterCommand("SetDumpLoad", BL09XX_SetDumpLoad, NULL); CMD_RegisterCommand("EnergyCntReset", BL09XX_ResetEnergyCounter, NULL);
    CMD_RegisterCommand("ToggleAuto", BL09XX_ToggleAuto, NULL); CMD_RegisterCommand("SetTargetPower", BL09XX_SetTargetPower, NULL);
    CMD_RegisterCommand("SetTargetExport", BL09XX_SetTargetExport, NULL); CMD_RegisterCommand("SetupEnergyStats", BL09XX_SetupEnergyStatistic, NULL);
    CMD_RegisterCommand("ConsumptionThreshold", BL09XX_SetupConsumptionThreshold, NULL); CMD_RegisterCommand("VCPPublishThreshold", BL09XX_VCPPublishThreshold, NULL);
    CMD_RegisterCommand("VCPPrecision", BL09XX_VCPPrecision, NULL); CMD_RegisterCommand("VCPPublishIntervals", BL09XX_VCPPublishIntervals, NULL);
}

float DRV_GetReading(energySensor_t type) { return sensors[type].lastReading; }
energySensorNames_t* DRV_GetEnergySensorNames(energySensor_t type) { return &sensors[type].names; }

// ====================================================================
// SEGMENTED JSON API ENDPOINT (Routed by "?q=" parameter)
// ====================================================================
int http_fn_api_dash(http_request_t *request) {
    http_setup(request, "application/json");
    poststr(request, "{");

    if (request->url != NULL && strstr(request->url, "q=net")) {
        if (CFG_HasFlag(OBK_FLAG_POWER_ALLOW_NEGATIVE) && NTP_IsTimeSynced()) {
            unsigned int minutes_since_midnight = NTP_GetHour() * 60 + NTP_GetMinute();
            char small_buf[128]; int pos = 0;
            poststr(request, "\"net\":[");
            for (int i = 47; i >= 0; i--) {
            ...
            poststr(request, "]");
        } else { poststr(request, "\"err\":1"); }
    }
    else if (request->url != NULL && strstr(request->url, "q=chg")) {
        if (CFG_HasFlag(OBK_FLAG_POWER_ALLOW_NEGATIVE) && NTP_IsTimeSynced()) {
            unsigned int minutes_since_midnight = NTP_GetHour() * 60 + NTP_GetMinute();
            char small_buf[128]; int pos = 0;
            poststr(request, "\"chg\":[");
    ...
    else if (request->url != NULL && strstr(request->url, "q=inv")) {
        if (CFG_HasFlag(OBK_FLAG_POWER_ALLOW_NEGATIVE) && NTP_IsTimeSynced()) {
            unsigned int minutes_since_midnight = NTP_GetHour() * 60 + NTP_GetMinute();
            char small_buf[128]; int pos = 0;
            poststr(request, "\"inv\":[");
        ...
        if (dmp == 0) { poststr(request, "\"chg_lbl\":\"Charger\",\"chg_v\":\"Idle\",\"chg_c\":\"#888\","); } 
        else if (dmp == 5) { poststr(request, "\"chg_lbl\":\"Charger\",\"chg_v\":\"Battery\",\"chg_c\":\"#4caf50\","); } 
        else { hprintf255(request, "\"chg_lbl\":\"Charging\",\"chg_v\":\"%d%%\",\"chg_c\":\"#0099FF\",", dmp); }
...
// ====================================================================
// HTML FRONTEND (Memory Chunked + JavaScript Round-Robin)
// ====================================================================
int http_fn_custom_dash(http_request_t *request) {
    ...
    poststr(request, "body { margin: 0; background-color: #000; display: -webkit-box; display: flex; -webkit-box-pack: center; justify-content: center; }");
    ...
    poststr(request, ".top-stats { display: -webkit-box; display: flex; -webkit-box-pack: justify; justify-content: space-between; -webkit-box-align: center; align-items: center; background: #222; padding: 18px; border-radius: 8px; text-align: center; margin-top: 15px; width: 100%; box-sizing: border-box; white-space: nowrap; }");
    poststr(request, ".top-stats div { display: -webkit-box; display: flex; -webkit-box-orient: vertical; flex-direction: column; -webkit-box-pack: center; justify-content: center; margin: 0 10px; }");
    poststr(request, ".top-stats label { color: #888; font-size: 20px; text-transform: uppercase; margin-bottom: 6px; display: block; } .top-stats b { font-size: 38px; font-weight: 600; }");
    poststr(request, ".c-exp { color: #4caf50; } .c-imp { color: #f44336; }");
    poststr(request, ".dash-row { display: -webkit-box; display: flex; -webkit-box-orient: horizontal; flex-direction: row; margin-top: 15px; height: 400px; -webkit-box-align: stretch; align-items: stretch; }");
    poststr(request, ".left-col { -webkit-box-flex: 0; flex: 0 0 230px; width: 230px; background: #222; padding: 10px; border-radius: 8px; overflow-y: auto; margin-right: 15px; box-sizing: border-box; }");
    ...
    poststr(request, ".graph-col { -webkit-box-flex: 1; flex: 1; background: #222; padding: 15px; border-radius: 8px; display: -webkit-box; display: flex; -webkit-box-align: center; align-items: center; -webkit-box-pack: center; justify-content: center; box-sizing: border-box; margin-right: 15px; overflow: hidden; }");
    poststr(request, "canvas { width: 100%; max-width: 592px; height: auto; display: block; margin: 0 auto; }");
    poststr(request, ".right-col { -webkit-box-flex: 0; flex: 0 0 240px; width: 240px; background: #222; padding: 20px; border-radius: 8px; display: -webkit-box; display: flex; -webkit-box-orient: vertical; flex-direction: column; box-sizing: border-box; }");
    ...
    poststr(request, ".bottom-clk-row { background: #222; border-radius: 8px; padding: 25px; margin-top: 15px; display: -webkit-box; display: flex; -webkit-box-orient: horizontal; flex-direction: row; -webkit-box-align: center; align-items: center; -webkit-box-pack: center; justify-content: center; box-sizing: border-box; width: 100%; }");
    poststr(request, ".clk-text-wrap { display: -webkit-box; display: flex; -webkit-box-orient: vertical; flex-direction: column; -webkit-box-align: start; align-items: flex-start; margin-left: 20px; text-align: left; }");
    ...
    if (CFG_HasFlag(OBK_FLAG_POWER_ALLOW_NEGATIVE)) {
        ...
        poststr(request, "<div style='margin-top:20px; font-size:14px; color:#eee; padding:15px; background:#1a1a1a; border-radius:6px; border:1px solid #333;'>");
        poststr(request, "<div style='margin-bottom:12px; color:#aaa; text-transform:uppercase; font-size:12px; font-weight:bold; letter-spacing:1px;'>Graph Legend</div>");
        poststr(request, "<div style='display:-webkit-box; display:flex; -webkit-box-align:center; align-items:center; margin-bottom:10px;'><span style='display:inline-block; width:18px; height:4px; background:#aaa; margin-right:12px;'></span><b>Total Energy</b></div>");
        poststr(request, "<div style='display:-webkit-box; display:flex; -webkit-box-align:center; align-items:center; margin-bottom:10px;'><span style='display:inline-block; width:18px; height:4px; background:#4caf50; margin-right:12px;'></span><b>Charger Avg</b></div>");
        poststr(request, "<div style='display:-webkit-box; display:flex; -webkit-box-align:center; align-items:center;'><span style='display:inline-block; width:18px; height:4px; background:#ff9800; margin-right:12px;'></span><b>Inverter Avg</b></div></div></div>");

        poststr(request, "<div class='graph-col'><div style='width:100%; max-width:592px; margin:0 auto;'><div style='position:relative; width:100%; padding-bottom:57.43%;'>");
        poststr(request, "<canvas id='dynCanvas' style='position:absolute; top:0; left:0; width:100%; height:100%;'></canvas>");
        poststr(request, "</div></div></div>");

        poststr(request, "<div class='right-col'><div style='font-size:12px; color:#888; text-transform:uppercase; margin-bottom:12px;'>System Modes</div>");
        poststr(request, "<button id='m-btn' class='btn-tgl' onclick='tm()'>--</button>");
        poststr(request, "<button id='inv-btn' class='btn-tgl' onclick='t_inv()'>INVERTER</button>");
        ...
        poststr(request, "<div class='sld-v-block'><label>Max Pwr (<span id='lbl-pwr'></span>%)</label><input type='range' id='sld-pwr' min='18' max='100' value='100' onchange='s_pwr(this.value)' style='width:100%;'></div>");
        poststr(request, "<div class='sld-v-block' style='margin-top:15px;'><label>Export (<span id='lbl-exp'></span> Wh)</label><input type='range' id='sld-exp' min='10' max='100' value='20' onchange='s_exp(this.value)' style='width:100%;'></div></div></div>");
        
        poststr(request, "<div class='bottom-clk-row'><div id='d-clk'>--:--</div><div class='clk-text-wrap'><div id='d-day'>--</div><div id='d-date'>--</div></div></div>");
    }

    // Dynamic Javascript (ES5 State Machine)
    poststr(request, "</div><script>");
    poststr(request, "var dmp=0, auto=0, netArr=[], chgArr=[], invArr=[];");
    poststr(request, "function setV(id,v){var e=document.getElementById(id);if(e){if(e.tagName==='INPUT')e.value=v;else e.innerHTML=v;}}");
    poststr(request, "function setC(id,v){var e=document.getElementById(id);if(e)e.className=v;}");
    poststr(request, "function setS(id,v){var e=document.getElementById(id);if(e)e.style.color=v;}");
    poststr(request, "function s_pwr(v){ var xhr=new XMLHttpRequest(); xhr.open('GET','/cm?cmnd=SetTargetPower%20'+v,true); xhr.send(); setV('lbl-pwr',v); }");
    poststr(request, "function s_exp(v){ var xhr=new XMLHttpRequest(); xhr.open('GET','/cm?cmnd=SetTargetExport%20'+v,true); xhr.send(); setV('lbl-exp',v); }");
    poststr(request, "function upd(v){if(auto===1)return; if(v>=18){setV('sld-pwr',v);} s_pwr(v); dmp=parseInt(v, 10); btnColor();}");
    ...
    poststr(request, "function btnColor(){");
    poststr(request, "var i=document.getElementById('inv-btn'),c=document.getElementById('chg-btn'),m=document.getElementById('m-btn');");
    poststr(request, "if(i) i.style.background=(dmp===5)?'#ff9800':'#555';");
    poststr(request, "if(c) c.style.background=(dmp>18)?'#4caf50':((dmp>=10&&dmp<=18)?'#8bc34a':'#555');");
    poststr(request, "if(m){ m.innerHTML=(auto===1)?'AUTO':'MANUAL'; m.style.background=(auto===1)?'#0099FF':'#f44336'; }");
    poststr(request, "}");

    poststr(request, "function drawSmooth(ctx, arr, baseY, clamp, fill, col, lw){");
    poststr(request, "var p=[]; for(var i=0; i<48; i++){ var h=(arr[i]||0)/2; if(clamp){if(h>150)h=150; if(h<-75)h=-75;} p.push({x:i*11+60, y:baseY-h}); }");
    poststr(request, "if(fill){ ctx.beginPath(); ctx.moveTo(p[0].x, p[0].y); for(var i=0; i<47; i++){ var xc=(p[i].x+p[i+1].x)/2, yc=(p[i].y+p[i+1].y)/2; ctx.quadraticCurveTo(p[i].x, p[i].y, xc, yc); }");
    poststr(request, "ctx.lineTo(p[47].x, p[47].y); ctx.lineTo(577,baseY); ctx.lineTo(60,baseY); ctx.fillStyle=fill; ctx.fill(); }");
    poststr(request, "ctx.beginPath(); ctx.moveTo(p[0].x, p[0].y); for(var i=0; i<47; i++){ var xc=(p[i].x+p[i+1].x)/2, yc=(p[i].y+p[i+1].y)/2; ctx.quadraticCurveTo(p[i].x, p[i].y, xc, yc); }");
    poststr(request, "ctx.lineTo(p[47].x, p[47].y); ctx.strokeStyle=col; ctx.lineWidth=lw; ctx.stroke();");
    poststr(request, "ctx.beginPath(); ctx.arc(p[47].x, p[47].y, lw*1.5, 0, 2*Math.PI); ctx.fillStyle=col; ctx.fill(); }");

    poststr(request, "function redrawG() {");
    poststr(request, "var c=document.getElementById('dynCanvas'); if(!c || !c.getContext) return;");
    poststr(request, "var ctx=c.getContext('2d'); var r=window.devicePixelRatio||1; c.width=Math.round(592*r); c.height=Math.round(340*r); ctx.scale(r,r); ctx.clearRect(0,0,592,340);");
    poststr(request, "ctx.fillStyle='#181818'; ctx.fillRect(60,10,517,50); ctx.fillRect(60,75,517,235); ctx.lineWidth=1; ctx.strokeStyle='#333'; ctx.beginPath();");
    poststr(request, "ctx.moveTo(60,35); ctx.lineTo(577,35); ctx.moveTo(60,75); ctx.lineTo(577,75); ctx.moveTo(60,150); ctx.lineTo(577,150); ctx.moveTo(60,300); ctx.lineTo(577,300); ctx.stroke();");
    poststr(request, "ctx.strokeStyle='#444'; ctx.beginPath(); ctx.moveTo(60,60); ctx.lineTo(577,60); ctx.moveTo(60,225); ctx.lineTo(577,225); ctx.stroke();");
    poststr(request, "ctx.strokeStyle='#666'; ctx.beginPath(); ctx.moveTo(60,10); ctx.lineTo(60,60); ctx.moveTo(60,75); ctx.lineTo(60,310); ctx.moveTo(577,10); ctx.lineTo(577,60); ctx.moveTo(577,75); ctx.lineTo(577,310); ctx.stroke();");
    poststr(request, "ctx.fillStyle='#aaa'; ctx.font='14px sans-serif'; ctx.textAlign='right'; ctx.textBaseline='middle'; ctx.fillText('100', 48, 10); ctx.fillText('0', 48, 60); ctx.fillText('+300', 48, 75); ctx.fillText('+150', 48, 150); ctx.fillStyle='#ccc'; ctx.fillText('0 Wh', 48, 225); ctx.fillStyle='#aaa'; ctx.fillText('-150', 48, 300);");
    poststr(request, "ctx.lineWidth=1; ctx.beginPath(); ctx.font='12px sans-serif'; ctx.textBaseline='top';");
    poststr(request, "for(var i=0; i<=47; i++){ var x=(47-i)*11+60; ctx.moveTo(x,310); if(i%4===0){ ctx.lineTo(x,316); ctx.textAlign=(i===0)?'right':'center'; ctx.fillText((i===0)?'Now':'-'+(i/4)+'h', x, 320); } else { ctx.lineTo(x,313); } } ctx.stroke();");
    poststr(request, "if(netArr.length > 0){ var grad=ctx.createLinearGradient(0,75,0,310); grad.addColorStop(0,'rgba(244,67,54,0.5)'); grad.addColorStop(0.638,'rgba(244,67,54,0.15)'); grad.addColorStop(0.638,'rgba(76,175,80,0.15)'); grad.addColorStop(1,'rgba(76,175,80,0.5)'); drawSmooth(ctx, netArr, 225, true, grad, '#aaa', 2.5); }");
    poststr(request, "if(chgArr.length > 0) drawSmooth(ctx, chgArr, 60, false, null, '#4caf50', 2.5);");
    poststr(request, "if(invArr.length > 0) drawSmooth(ctx, invArr, 60, false, null, '#ff9800', 2.5);");
    poststr(request, "}");

    // Segemented Fetch Function
    poststr(request, "function fetchStep(q){ var x = new XMLHttpRequest();");
    poststr(request, "x.onreadystatechange = function() { if(x.readyState === 4 && x.status === 200) { try { var d = JSON.parse(x.responseText);");
    
    poststr(request, "if(q === 'base') { setV('d-va',d.va); setV('d-pwr',d.pwr); setC('d-pwr',d.pwr_cls); setV('d-bal',d.bal); setC('d-bal',d.bal_cls);");
    poststr(request, "setV('d-est',d.est); setC('d-est',d.est_cls); setV('c-lbl',d.chg_lbl); setV('c-v',d.chg_v); setS('c-v',d.chg_c); setV('d-clk',d.clk);");
    poststr(request, "if(d.t_pwr>=18) setV('sld-pwr',d.t_pwr); setV('lbl-pwr',d.t_pwr); setV('sld-exp',d.t_exp); setV('lbl-exp',d.t_exp);");
    poststr(request, "dmp=d.dmp; auto=d.auto; btnColor(); if(init) setTimeout(function(){fetchStep('sens');}, 200); }");
    
    poststr(request, "else if(q === 'sens') { if(d.sens) setV('d-sens-body',d.sens); if(init) setTimeout(function(){fetchStep('net');}, 200); }");
    poststr(request, "else if(q === 'net') { if(d.net){ netArr=d.net; redrawG(); } if(init) setTimeout(function(){fetchStep('chg');}, 200); }");
    poststr(request, "else if(q === 'chg') { if(d.chg){ chgArr=d.chg; redrawG(); } if(init) setTimeout(function(){fetchStep('inv');}, 200); }");
    poststr(request, "else if(q === 'inv') { if(d.inv){ invArr=d.inv; redrawG(); } if(init){ init=false; startTimers(); } }");
    poststr(request, "} catch(e) {} } }; x.open('GET','/api_dash?q='+q+'&t='+Date.now(),true); x.send(); }");

    // Independent Timers Setup
    poststr(request, "function startTimers() {");
    poststr(request, "setInterval(function(){fetchStep('base');}, 10000);"); // Base stats: Every 10 seconds
    poststr(request, "setInterval(function(){fetchStep('sens');}, 30000);"); // Table text: Every 30 seconds
    poststr(request, "setInterval(function(){fetchStep('net');}, 60000);");  // Net graph: Minute mark 0
    poststr(request, "setTimeout(function(){ setInterval(function(){fetchStep('chg');}, 60000); }, 20000);"); // Chg graph: Minute mark 20
    poststr(request, "setTimeout(function(){ setInterval(function(){fetchStep('inv');}, 60000); }, 40000);"); // Inv graph: Minute mark 40
    poststr(request, "}");

    // Initialize Local Clock
    poststr(request, "var days=['SUNDAY','MONDAY','TUESDAY','WEDNESDAY','THURSDAY','FRIDAY','SATURDAY']; var mos=['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];");
    poststr(request, "function runClock(){ var dt=new Date(); setV('d-day', days[dt.getDay()]); setV('d-date', mos[dt.getMonth()]+' '+dt.getDate()+', '+dt.getFullYear()); }");
    poststr(request, "runClock(); setInterval(runClock, 60000);");

    // Ignite 
    poststr(request, "var init = true; fetchStep('base');");
