...
void BL09XX_AppendInformationToHTTPIndexPage(http_request_t *request)
{
    ...
    if (CFG_HasFlag(OBK_FLAG_POWER_ALLOW_NEGATIVE) && NTP_IsTimeSynced())
    {
        ...
        // ====================================================================
        // 3. BAR GRAPH 8-HOURS (Right Column)
        // ====================================================================
        poststr(request, "<div style='flex:1; min-width:300px; margin-right:20px;'>");
        poststr(request, "<h3 style='font-size:16px; margin:0 0 10px 0;'>Energy Stats (Last 8 Hours)</h3>");
        // Box: Transparent background with a 1px solid white border
        poststr(request, "<div style='position:relative; height:240px; background:transparent; border:1px solid #fff; padding:0 2%; overflow:hidden;'>");
        
        // Central Zero line fixed at 30% from the bottom
        poststr(request, "<div style='position:absolute; bottom:30%; left:0; right:0; height:1px; background:#fff; z-index:1;'></div>");
        ...
        for (int i = 31; i >= 0; i--) {
            ...
            // Total scale is 1000W
            float h_pct = (abs(display_net) * 100.0f) / 1000.0f; 
            if (h_pct < 0.5f && display_net != 0) h_pct = 0.5f;     
            
            // Horizontally distribute the 32 columns
            float left_pct = 2.0f + ((31 - i) * 96.0f) / 31.0f;

            if (display_net > 0) {
                // Import (Positive) -> Green bar growing UP from the 30% line
                hprintf255(request, "<div style='position:absolute; bottom:30%%; left:%.1f%%; width:8px; height:%.1f%%; background:#4caf50; transform:translateX(-50%%); z-index:2;'>", left_pct, h_pct);
                hprintf255(request, "<span style='position:absolute; bottom:100%%; left:50%%; transform:rotate(-90deg); transform-origin:left center; font-size:10px; color:#fff; white-space:nowrap; margin-left:-4px; margin-bottom:4px;'>%dW</span></div>", net);
            } else if (display_net < 0) {
                // Export (Negative) -> Red bar growing DOWN from the 30% line (top: 70%)
                hprintf255(request, "<div style='position:absolute; top:70%%; left:%.1f%%; width:8px; height:%.1f%%; background:#f44336; transform:translateX(-50%%); z-index:2;'>", left_pct, h_pct);
                hprintf255(request, "<span style='position:absolute; top:100%%; left:50%%; transform:rotate(90deg); transform-origin:left center; font-size:10px; color:#fff; white-space:nowrap; margin-left:-4px; margin-top:4px;'>%dW</span></div>", net);
            }
        }
        ...
        if (current_minute != last_minute) 
        {
            ...
            for (int output_index = 0; output_index < dump_load_relay_number; output_index++) 
            {
                if (dump_load_relay[output_index] != last_dump_load_relay[output_index]) 
                {
                    ...
                    {
                        // COMMENTED OUT HTTP SendGet for Charger C
                        /*
