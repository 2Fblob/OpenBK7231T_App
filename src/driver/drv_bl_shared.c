...
int http_fn_custom_dash(http_request_t *request) {
    ...
    poststr(request, 
        ...
        // --- THE SEQUENTIAL 20-SECOND POLLER ---
        "function refresh(){"
        ...
        // Store incoming arrays locally so canvas can redraw anytime
        ...
        // Rotate state: 0 (Core) -> 1 (Net) -> 2 (Chg) -> 3 (Inv) -> 0
        "poll_state++;"
        "if (poll_state > 3) poll_state = 0;"
        "}"
        
        "refresh(); setInterval(refresh, 15000);" // 15 seconds per call = exactly 60 seconds per full cycle
        "</script></body></html>"
    );
    poststr(request, NULL);
    return 0;
