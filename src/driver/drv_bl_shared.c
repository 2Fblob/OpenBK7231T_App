...
// COMPILER ERROR FIX: Included HTTP structures to define http_request_t
...
void BL09XX_AppendInformationToHTTPIndexPage(http_request_t *request)
{
    ...
    poststr(request, "#state, form[action*='tgl'] { display:none !important; }");
    poststr(request, ".b-txt { color:#fff; font-size:10px; font-weight:bold; writing-mode:vertical-rl; transform:rotate(180deg); overflow:visible; display:block; padding:2px; }");
    poststr(request, ".g-top { height:140px; position:relative; } .g-bot { height:60px; position:relative; }");
    poststr(request, ".g-up { position:absolute; bottom:0; width:100%; background:#e74c3c; display:flex; align-items:center; justify-content:center; }");
    poststr(request, ".g-dn { position:absolute; top:0; width:100%; background:#2ecc71; display:flex; align-items:center; justify-content:center; }");
    poststr(request, ".g-b { width:19px; margin-right:1px; height:100%; display:flex; flex-direction:column; }");
    poststr(request, "</style>");

    // ====================================================================
    // 1. HORIZONTAL DASHBOARD 
    // ====================================================================
    poststr(request, "<hr><table style='width:100%; text-align:center; font-size:14px; margin-top:10px; margin-bottom:15px;'>");
    ...
    poststr(request, "<th style='color:#aaa; font-weight:normal; padding-bottom:5px;'>15-Min Est.</th>");
    ...
    hprintf255(request, "<tr><td><b>%.0f V</b></td><td><b>%.0f W</b></td><td><b>%i Wh</b></td><td><b style='color:#0099FF;'>%i%%</b></td><td><b>%s</b></td></tr></table>", 
               sensors[OBK_VOLTAGE].lastReading, 
               sensors[OBK_POWER].lastReading, 
               estimated_energy_period,
               dump_load_relay[5],
               solar_available ? "Exporting" : "Importing");

    // ====================================================================
    // 2. LIVE NETMETERING (15-MIN BUCKET)
    // ====================================================================
    if (CFG_HasFlag(OBK_FLAG_POWER_ALLOW_NEGATIVE) && NTP_IsTimeSynced())
    {
        ...
        poststr(request, "<table style='width:100%; text-align:center; margin-bottom:15px;'>");
        poststr(request, "<tr><th style='text-align:left;'>Time</th><th>Import</th><th>Export</th><th>Net</th></tr>");
        hprintf255(request, "<tr><td style='text-align:left;'><b>%02i:%02i</b></td><td><b>%dW</b></td><td><b>%dW</b></td><td><b>%dW</b></td></tr></table>", 
                   hour, minute, disp_cons, disp_exp, disp_net);

        // ====================================================================
        // 3. THE BAR GRAPH (LAST 6 HOURS / 24 BARS)
        // ====================================================================
        poststr(request, "<br><h2>Energy Stats (Last 6 Hours)</h2>");
        poststr(request, "<div style='display:flex; height:201px; max-width:480px; margin:auto; font-family:sans-serif;'>");
        
        // Y-Axis Scale
        poststr(request, "<div style='width:45px; display:flex; flex-direction:column; justify-content:space-between; font-size:10px; color:#aaa; text-align:right; padding-right:8px;'><span>+700W</span><span>0W</span><span>-300W</span></div>");
        
        // Inner Graph Canvas (Relative for absolute positioning the zero line)
        poststr(request, "<div style='flex-grow:1; display:flex; align-items:center; justify-content:flex-start; background:#2a2a2a; position:relative;'>");
        poststr(request, "<div style='position:absolute; top:140px; width:100%; height:1px; background:#666;'></div>"); // 0-Line

        // Draw 24 horizontal slots
        for (int i = 23; i >= 0; i--) {
            ...
            if (i == 0) { v += (int)(real_consumption - real_export); } // Include live data for newest bar
            
            poststr(request, "<div class='g-b'>");
            
            // Top Half (Import/Red) - 140px tall corresponding to +700W scale
            if (v > 0) {
                int h_up = (v * 140) / 700;
                if (h_up > 140) h_up = 140;
                if (h_up < 2) h_up = 2; // Min thickness
                hprintf255(request, "<div class='g-top'><div class='g-up' style='height:%dpx;'><span class='b-txt'>%d</span></div></div>", h_up, v);
            } else {
                poststr(request, "<div class='g-top'></div>");
            }
            
            // Bottom Half (Export/Green) - 60px tall corresponding to -300W scale
            if (v < 0) {
                int h_dn = (abs(v) * 60) / 300;
                if (h_dn > 60) h_dn = 60;
                if (h_dn < 2) h_dn = 2; // Min thickness
                hprintf255(request, "<div class='g-bot'><div class='g-dn' style='height:%dpx;'><span class='b-txt'>%d</span></div></div>", h_dn, v);
            } else {
                poststr(request, "<div class='g-bot'></div>");
            }
            poststr(request, "</div>");
        }
        ...
        // ====================================================================
        poststr(request, "<h3>Detailed NetMetering History</h3><hr>");
        poststr(request, "<table style='width:100%; text-align:center;'>");
        poststr(request, "<tr><th style='text-align:left;'>Time</th><th>Net</th></tr>");

        // Loop the remaining 47 intervals (last 12 hours)
        for (int i = 1; i < 48; i++) { 
            int interval_of_day = current_interval_of_day - i;
            if (interval_of_day < 0) { interval_of_day += 96; } 
            ...
            hprintf255(request, "<tr><td style='text-align:left;'>%02i:%02i</td><td>%dW</td></tr>", h, m, d_net);
        }
    ...
    // JS TELEPORT HACK
    // ====================================================================
    poststr(request, "<script>setTimeout(function(){ var s=document.getElementById('state'); var m=document.getElementById('main'); if(s && m){ s.style.display='block'; m.appendChild(s); } }, 200);</script>");
}
