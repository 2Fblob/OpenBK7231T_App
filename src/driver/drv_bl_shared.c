...
void BL09XX_AppendInformationToHTTPIndexPage(http_request_t *request)
{
    ...
    if (CFG_HasFlag(OBK_FLAG_POWER_ALLOW_NEGATIVE) && NTP_IsTimeSynced())
    {
        ...
        // ====================================================================
        // 3. CANVAS BAR GRAPH 8-HOURS (Right Column) - CLIENT SIDE RENDERED
        // ====================================================================
        poststr(request, "<div style='flex:1; min-width:300px; margin-right:20px; display:flex; align-items:flex-end;'>");
        
        // Output a blank Canvas element instead of 32 SVG nodes
        poststr(request, "<canvas id='chart' width='480' height='320' style='width:100%; max-width:480px; border:1px solid #000;'></canvas>");
        ...
        for (int i = 31; i >= 0; i--) {
            ...
            // Just print the number and a comma
            hprintf255(request, "%d%s", net, (i == 0) ? "" : ",");
        }
        poststr(request, "];");

        // Append a minified script to let the browser draw the chart
        poststr(request, "const c=document.getElementById('chart').getContext('2d');"
                         "c.beginPath();c.moveTo(0,230);c.lineTo(480,230);c.strokeStyle='#fff';c.stroke();"
                         "c.font='8px sans-serif';c.textAlign='start';c.textBaseline='middle';"
                         "d.forEach((v,i)=>{ let x=i*15+7; "
                         "if(v>=0){ let h=Math.max((Math.min(v,700)*190)/700, 1); c.fillStyle='#f44336'; c.fillRect(x-5, 230-h, 11, h); "
                         "if(v>0){ c.save(); c.translate(x, 230-h-3); c.rotate(-Math.PI/2); c.fillStyle=i===31?'#0099FF':'#fff'; c.fillText(v,0,0); c.restore(); } }"
                         "else { let h=Math.max((Math.abs(Math.max(v,-300))*60)/300, 1); c.fillStyle='#4caf50'; c.fillRect(x-5, 230, 11, h); "
                         "if(v<0){ c.save(); c.translate(x, 230+h+3); c.rotate(-Math.PI/2); c.textAlign='end'; c.fillStyle=i===31?'#0099FF':'#fff'; c.fillText(v,0,0); c.restore(); } }"
                         "});</script>");
