...
void BL_ProcessUpdate(float voltage, float current, float power, float frequency, float energyWh) {
    ...
    for(i = OBK__FIRST; i <= OBK__LAST; i++)
    {
            ...
            (sensors[i].noChangeFrame >= changeSendAlwaysFrames) )
        {
            ...
            {
                int prev_mA = (int)(sensors[i].lastSentValue * 1000);
                // Hook logic or log messages for current change if required here
                break;
            }
            default:
                break;
            ...
            // Publish updated values over MQTT
            float valToSend = sensors[i].lastReading;
            if (i >= OBK_CONSUMPTION_TOTAL && i <= OBK_CONSUMPTION_3_DAYS_AGO) {
                valToSend = BL_ChangeEnergyUnitIfNeeded(valToSend);
            }
            MQTT_PublishMain_Float(sensors[i].names.pName, valToSend, sensors[i].rounding_decimals);
...
// Initializer registration pattern for driver setup inside OpenBeken framework
void BL09XX_Init() {
    CMD_RegisterCommand("ResetEnergyCounter", BL09XX_ResetEnergyCounter, NULL);
    ...
    CMD_RegisterCommand("SetupEnergyStatistic", BL09XX_SetupEnergyStatistic, NULL);
    ...
    CMD_RegisterCommand("SetupConsumptionThreshold", BL09XX_SetupConsumptionThreshold, NULL);
}
