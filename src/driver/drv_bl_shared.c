...
void BL_ProcessUpdate(float voltage, float current, float power, float frequency, float energyWh) {
    ...
    {                                          
        ...
        if (current_minute != last_minute) 
        {
            ...
            // ** NEW: Import Discharge Logic (0 to 5 Range) **
            if (current_net_power_w > 0) {
                int calc_val = 5 - (current_net_power_w / 10);
                if (calc_val < 4) {
                    dump_load_relay[5] = 0;  // Jump straight to 0 if < 4
                } else if (dump_load_relay[5] == 0) {
                    // Hold at 0 until equivalent export reaches >= 2.5Wh (-2 int)
            ...
            // ** Existing Export Charge Logic (5 to 100 Range) **
            else if (change != 0) { 
                if (current_net_power_w <= -50 || (dump_load_relay[5] >= 5 && change < 0)) {
                    dump_load_relay[5] += change;
                }
                if (dump_load_relay[5] > 5 && dump_load_relay[5] < 10) {dump_load_relay[5] = 10;}
            }
            ...
            if (current_net_power_w < 0 && current_net_power_w > -51) {
                if (dump_load_relay[5] > 5) dump_load_relay[5] = 10;  
            }
