...
void BL09XX_AppendInformationToHTTPIndexPage(http_request_t *request)
{
    ...
    if (CFG_HasFlag(OBK_FLAG_POWER_ALLOW_NEGATIVE) && NTP_IsTimeSynced())
    {
        ...
        // Canvas is 320px tall to add 20px depth. Border is #000 (black).
        poststr(request, "<svg viewBox=\"0 0 480 320\" style=\"width:100%; max-width:480px; height:auto; background:transparent; border:1px solid #000;\" xmlns=\"http://www.w3.org/2000/svg\">");
        
        // Inverted axes: Central Zero line fixed near the bottom at y=230.
        poststr(request, "<line x1=\"0\" y1=\"230\" x2=\"480\" y2=\"230\" stroke=\"#fff\" stroke-width=\"1\"/>");
        ...
        for (int i = 31; i >= 0; i--) {
            ...
            // X positioning perfectly distributed across 480px (15px gap logic)
            int x = (31 - i) * 15 + 2; 
...
void BL_ProcessUpdate(float voltage, float current, float power, float frequency, float energyWh) {
    ...
    {                                          
        ...
        if (current_minute != last_minute) 
        {
            ...
            int minutes_remaining = 15 - min_in_block; 
            
            // Keep this calculation intact for the Web UI Dashboard display
            estimated_energy_period = (int)net_energy + ((int)sensors[OBK_POWER].lastReading * minutes_remaining) / 60;

            // ** NEW LOGIC: Time-Aware Hysteresis & Dynamic Export Sink **

            if (net_energy > 2.0f) {
                // Zone 1: Import Recovery - Reset to baseline
                dump_load_relay[5] = 5;
            } 
            else if (net_energy >= -10.0f) {
                // Zone 2: Ramp-Down Hysteresis (-10Wh to 2Wh)
                if (dump_load_relay[5] >= 11) {
                    dump_load_relay[5] = 11; // Hold charge level if clouds rolled in
                } else {
                    dump_load_relay[5] = 0;  // Stay off if dropping from baseline import
                }
            }
            else if (net_energy >= -20.0f) {
                // Zone 3: Export Deadband (-20Wh to -10Wh)
                if (dump_load_relay[5] >= 18) {
                    dump_load_relay[5] = 18; // Hold at 18 if recovering from deeper export
                } else {
                    dump_load_relay[5] = 0;  // Remain OFF while export is just starting
                }
            }
            else if (net_energy > -50.0f) {
                // Zone 4: Initial Charge Trigger (-49Wh to -21Wh)
                dump_load_relay[5] = 18;
            }
            else {
                // Zone 5: Dynamic Charge (<= -50Wh)
                // Equivalent Power (W) = abs(net_energy) * (60 / minutes_remaining)
                // Command = Equivalent Power / 10
                float equivalent_power = fabs(net_energy) * (60.0f / (float)minutes_remaining);
                dump_load_relay[5] = (int)(equivalent_power / 10.0f);
            }
            
            // Safety boundaries (clamp output strictly to 0-100)
            if (dump_load_relay[5] > 100) {
                dump_load_relay[5] = 100;
            } else if (dump_load_relay[5] < 0) {
                dump_load_relay[5] = 0;
            }

            // ** Send Unconditionally EVERY Minute, EXCLUSIVELY to Charger C **
            char dgr_cmd[64];
