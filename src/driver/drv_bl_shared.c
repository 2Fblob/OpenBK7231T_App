    ...
    if (CFG_HasFlag(OBK_FLAG_POWER_ALLOW_NEGATIVE) && NTP_IsTimeSynced())
    {
        ...
        for (int i = 15; i >= 0; i--) {
            ...
            if (v != 0 || i == 0) {
                ...
                hprintf255(request, "<text x='%d' y='%d' fill='#ddd' font-size='16' font-family='sans-serif' text-anchor='middle'>%d</text>", x + 16, text_y, v);
            } else {
                hprintf255(request, "<text x='%d' y='135' fill='#555' font-size='16' font-family='sans-serif' text-anchor='middle'>0</text>", x + 16);
            }
        }
