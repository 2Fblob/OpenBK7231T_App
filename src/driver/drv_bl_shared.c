...
void BL09XX_AppendInformationToHTTPIndexPage(http_request_t *request)
{
    ...
    if (CFG_HasFlag(OBK_FLAG_POWER_ALLOW_NEGATIVE) && NTP_IsTimeSynced())
    {
        ...
        poststr(request, 
            "function upd(v){if(auto===1)return; dmp=parseInt(v);var xhr=new XMLHttpRequest();"
            ...
            "if(sys){"
            "dmp=parseInt(sys.getAttribute('data-dmp'));"
            "auto=parseInt(sys.getAttribute('data-auto'));"
            "}"
